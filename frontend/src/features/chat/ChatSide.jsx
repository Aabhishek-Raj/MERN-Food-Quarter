...
import { accessChat, createChat } from './chatSlice'
...
const ChatSide = () => {
    ...
    const manage = user ? 'USER' : 'SUPPLIER';
                ...
                <ul class="flex flex-col w-full h-screen px-2 select-none">

                    {
                        searchResult && searchResult.map(each => (
                            <SearchChatCard key={each._id} user={each} handleFunction={() => dispatch(createChat(each._id))} />
                        ))
                    }
                    {
                        chats && chats.map(chat => (
                            <ChatCard key={chat._id} chat={chat} handleFunction={handleFunction} />
                        ))      
                    }
